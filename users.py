...
from good import Main_User, Main_UserDB, New_Response
import hashlib
...
@users_router.get("/api/users/{id}", response_model=Union[Main_User, New_Response])
def get_user(id: int):
    user = find_user(id)
    print(user)
    if user == None:
...
@users_router.post("/api/users", response_model=Union[Main_User, New_Response])
...
@users_router.put("/api/users", response_model=Union[Main_User, New_Response])
